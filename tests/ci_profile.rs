...
fn the_pr_gate_builds_fixtures_once_in_the_harness_vm_and_tests_both_architectures_through_chore() {
    ...
    // NO `fixtures` JOB, AND THAT IS THIS REPOSITORY'S SHAPE RATHER THAN AN
    // OMISSION. The sibling drivers generate a matrix of images with their
    // mkfs before the suite runs, so their test job depends on that
    // artifact. Here the only committed fixture is
    // test-disks/squashfs-basic.sqfs, and every other image is built by the
    // test that reads it, inside the guest, from a tree it staged itself.
    //
    // Asserted rather than left silent, so that adding a `needs: fixtures`
    ...
    // AND IT DOWNLOADS NO `fixtures` ARTIFACT, for the same reason it needs
    // no `fixtures` job: nothing here produces one. The inverse is asserted
    // so that a step copied from a sibling fails rather than silently
    // waiting on an artifact that will never be uploaded.
    assert!(
        !steps
            .iter()
            .any(|step| is_artifact_step(step, "download-artifact", "fixtures")),
        "jobs.test downloads a `fixtures` artifact, but no job in this repository \
         uploads one: the images are built in the guest by the tests that read them"
    );
    ...
    assert!(
        steps
            .iter()
            .all(|step| !run_of(step).contains("squashfs-tools")),
        "jobs.test must not install squashfs-tools on the runner: the tools live in the VM"
    );
    // SPLIT ON PURPOSE, like every other tool name in this file: written
    // whole it would be a literal oracle-tool name inside a test source,
    // which tests/test_contract.rs scans for and would report as a test
    // reaching for a tool on the host.
    let checker = ["unsquash", "fs"].concat();
    assert!(
        steps
            .iter()
            .any(|step| run_of(step).contains(&checker) && run_of(step).contains("::error::")),
        "jobs.test must make any oracle tool on the runner UNUSABLE before the tests \
         run, so that a green run is evidence every call went to the guest rather than \
         a claim that it did"
    );
    ...
    // THERE IS NO jobs.fixtures, AND THAT IS ASSERTED RATHER THAN ASSUMED.
    //
    // The sibling drivers build a matrix of images with their mkfs in a
    // dedicated job and pass it on as an artifact. This crate's only
    // committed image is test-disks/squashfs-basic.sqfs; every other image
    // is built inside the guest by the test that reads it, from a tree that
    // test staged itself.
    //
    // Checked in the negative because the failure it prevents is a job
    // copied across from a sibling: one that runs `chore fixtures` — a task
    // that does not exist here — or uploads an artifact nothing downloads,
    // and costs a VM boot per run to produce nothing.
    assert!(
        field(&document, "jobs")
            .and_then(|jobs| field(jobs, "fixtures"))
            .is_none(),
        "ci.yml has a jobs.fixtures, but this repository generates no fixtures and has \
         no `chore fixtures` task: the images are built in the guest by the tests that \
         read them"
    );
    ...
    // ONE PRE-FLIGHT CHECK, NOT TWO. The sibling drivers also require
    // `build-fixtures.sh --check` here, so that a missing generated image
    // fails once rather than in every test that opens one. This crate
    // generates none — see the note about jobs.fixtures above — so the only
    // thing to check before the tiers run is the host's own tools.
    let check = "scripts/tools.sh --check";
    assert!(
        commands.iter().any(|c| c.trim() == check),
        "chores.yml `test:native` must run `{check}` first, so a missing tool fails \
    ...
    assert!(
        commands.iter().any(|c| {
            ...
            as_cargo.contains("cargo test")
                ...
                && as_cargo.contains("test-targets.sh all")
        }),
        "chores.yml `test:native` must run the whole suite in the release profile, \
         selected with `scripts/test-targets.sh all`; it runs {commands:?}"
    );
    assert!(
...
fn the_release_gate_builds_fixtures_in_the_harness_vm_and_runs_chore_test() {
    ...
    // The VM must be ready before the suite asks for it. There is no
    // `chore fixtures` here to order against — see the note in the PR-gate
    // test above — so the thing that needs the host set up is `chore test`
    // itself, whose oracle and kernel tiers run inside the guest.
    let build = steps
        .iter()
        .position(|step| chore_invocations(run_of(step)).iter().any(|t| t == "test"))
        .unwrap_or_else(|| panic!("release jobs.test must run `chore test`"));
    assert!(
        setup < build,
        "release jobs.test must set up the VM host before `chore test` needs it: the \
         oracle and kernel tiers run inside the guest"
    );
    assert!(
    ...
    for (name, body) in jobs.iter() {
        ...
        {
            let run = run_of(step);
            // None of what the harness replaced may come back: the
            // 60-line squashfs-tools source build that stood in this
            // workflow and in ci.yml, and the `--ignored` opt-in that
            // was how the oracles were selected before they became a
            // tier of their own.
            for deleted in [
                "squashfs-tools-1.9.1",
                "--enable-fuse",
                "-- --ignored",
                "scripts/vm.sh",
            ] {
