...
#
# A gate that cannot fail is indistinguishable from no gate. Note how `check`
# captures `$?` from a plain command and never from a pipe: `$?` after a pipe
# is the exit status of the pipe's LAST command, so `... | grep -q` would
